            ...
            {(mode === 'reject' || mode === 'revise') && (
              <div className="mt-6 pt-4 border-t border-[var(--border)]">
                ...
                <div className="flex gap-2 mt-3">
                  {mode === 'reject' && <button onClick={() => data.requireAuth(() => { type === "topic" ? data.rejectTopic(item.id, feedbackText) : type === "draft" ? data.rejectDraft(item.id, feedbackText) : null; setModal(null); setFeedbackText(''); })} disabled={!feedbackText.trim()} className="px-4 py-2.5 bg-[var(--red)] text-white rounded-xl text-sm font-semibold hover:opacity-90 disabled:opacity-40 transition-all">Reject</button>}
